...
export default function Home() {
  ...
  const loadPosts = useCallback(async (isLoadingMore = false) => {
    if (isLoadingMore) {
      setLoadingMore(true);
    } else {
      setLoading(true);
    ...
    try {
      const [postsData, info] = await Promise.all([
        fetchRedditPosts(selectedSubreddit, sortBy, isLoadingMore ? after : undefined),
        !isLoadingMore ? fetchSubredditInfo(selectedSubreddit) : Promise.resolve(subredditInfo)
      ]);
      ...
      if (message === 'Subreddit not found') {
        setError(`Looks like r/${selectedSubreddit} doesn't exist. Double-check the spelling or try searching for a different subreddit!`);
      } else {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className={`flex-1 transition-all duration-300 ${sidebarCollapsed ? 'md:ml-[60px]' : 'md:ml-[300px]'}`}>
